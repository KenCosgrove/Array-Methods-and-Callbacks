...
function getWinnersByYear(cb, cb2) {
  for (let i = 0; i < cb.length; i++) {
    console.log(`In ${cb[i]}, ${cb2[i]} won the World Cup!`);
  }
}
...
function getCountryWins(data, initials) {
  data.reduce((acc, item) => {
    return acc + item[initials];
  });
}

console.log(getCountryWins(fifaData, "ARG"));

/* Stretch 3: Write a function called getGoals() that 
accepts a parameter `data` and returns the team with the most
...
/* Stretch 4: Write a function called badDefense() that accepts 
a parameter `data` and calculates the team with the most goals
...
/* If you still have time, use the space below to work on any stretch
 goals of your chosing as listed in the README file. */
